...
/**
 * תוספת נוקלאופילית פשוטה לקרבוניל (אלדהיד/קטון). שלב אחד, שני פאנלים: מגיבים
 * Nu⁻ + R(C=O)R'/H (קרבוניל שטוח, sp2) -> תוצר אלקוקסיד טטראהדרלי (sp3 אמיתי - מרכז
 * סטריאוגני חדש עם 4 קבוצות שונות: R, R'/H, Nu, O⁻). לכן בפאנל התוצר משתמשים בטריז
 * ובמקווקוו (כמו ב-NucleophilicSubstitutionArt) כדי להראות את הגיאומטריה הטטראהדרלית
 * החדשה - R ו-R'/H מצוירים כטריז/מקווקוו (כמו ה-H-ים בתוצר ה-SN2), בעוד Nu ו-O⁻ (הקבוצות
 * החשובות למנגנון) נשארים כקווים רגילים במישור לשם קריאות התוויות.
 */
export function AldehydesKetonesArt() {
  return (
    <MechanismArtFrame
      title="תוספת נוקלאופילית לקרבוניל (אלדהיד/קטון) - ללא שלב אלימינציה"
      note="החץ הכחול: זוג האלקטרונים הבודד של הנוקלאופיל תוקף את פחמן הקרבוניל האלקטרופילי. החץ השחור: זוג האלקטרונים של הקשר הכפול C=O נודד אל החמצן ויוצר מטען שלילי (O⁻). הפחמן עובר מ-sp2 שטוח ל-sp3 טטראהדרלי (טריז/מקווקוו בתוצר). בניגוד לחומצות קרבוקסיליות ונגזרותיהן (שיש בהן קבוצה עוזבת שיכולה לצאת ולהחזיר את הקרבוניל), כאן R ו-R'/H אינם קבוצות עוזבות טובות - לכן זו תוספת סופית (התוצר הוא אלקוקסיד/אלכוהול לאחר פרוטונציה, לא נגזרת חדשה)."
    >
      <svg viewBox="0 0 720 220" className="w-full h-auto" style={{ minWidth: 560 }}>
        ...
        {/* ===== פאנל 1: מגיבים - קרבוניל שטוח (sp2) ===== */}
        <g fontFamily="inherit">
          ...
          {/* Nucleophile approaching from the left, with lone pair */}
          <ChemLatexSvg tex="\ce{Nu^-}" x={58} y={148} fontSize={17} color={MECH_COLORS.nucleophile} />
        ...
        {/* ===== פאנל 2: תוצרים - אלקוקסיד טטראהדרלי (sp3 אמיתי, טריז/מקווקוו) ===== */}
        <g fontFamily="inherit">
          {/* R group - wedge (out of the page) */}
          <WedgeBond x1={520} y1={108} x2={485} y2={80} />
          <text x="472" y="72" fontSize="15" fill={MECH_COLORS.atom} textAnchor="middle">R</text>
          ...
          {/* C-O(-) single bond, up - plain, in plane */}
          ...
          <ChemLatexSvg tex="^-" x={538} y={38} width={26} fontSize={14} color={MECH_COLORS.charge} anchor="start" />

          {/* R'/H group - dash (behind the page) */}
          <DashBond x1={520} y1={108} x2={555} y2={135} />
          <text x="574" y="152" fontSize="15" fill={MECH_COLORS.atom} textAnchor="middle">R'/H</text>

          {/* new C-Nu bond - plain, in plane */}
